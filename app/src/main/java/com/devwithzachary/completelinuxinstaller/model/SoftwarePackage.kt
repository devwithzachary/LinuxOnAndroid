        ...
        fun getPresets(): List<SoftwarePackage> {
            return listOf(
                ...
                SoftwarePackage(
                    ...
                    iconName = "Security",
                    installCommand = "$NONINT_EXPORT && dpkg --configure -a && apt-get update $DPKG_FLAGS && apt-get install -y $DPKG_FLAGS openssh-server && mkdir -p /run/sshd /var/run/sshd /var/empty /etc/ssh/sshd_config.d && [ -f /etc/ssh/ssh_host_rsa_key ] || ssh-keygen -t rsa -f /etc/ssh/ssh_host_rsa_key -N \"\" && [ -f /etc/ssh/ssh_host_ecdsa_key ] || ssh-keygen -t ecdsa -f /etc/ssh/ssh_host_ecdsa_key -N \"\" && [ -f /etc/ssh/ssh_host_ed25519_key ] || ssh-keygen -t ed25519 -f /etc/ssh/ssh_host_ed25519_key -N \"\" && ssh-keygen -A 2>/dev/null || true && sed -i 's/^UsePAM.*/UsePAM no/' /etc/ssh/sshd_config 2>/dev/null || true && sed -i 's/^#UsePAM.*/UsePAM no/' /etc/ssh/sshd_config 2>/dev/null || true && sed -i 's/.*pam_systemd.so.*/# &/' /etc/pam.d/sshd /etc/pam.d/common-session 2>/dev/null || true && echo \"UsePAM no\" > /etc/ssh/sshd_config.d/00-linuxonandroid.conf && echo \"PrintMotd no\" >> /etc/ssh/sshd_config.d/00-linuxonandroid.conf && echo \"PrintLastLog no\" >> /etc/ssh/sshd_config.d/00-linuxonandroid.conf && echo \"PermitRootLogin yes\" >> /etc/ssh/sshd_config.d/00-linuxonandroid.conf && echo \"PasswordAuthentication yes\" >> /etc/ssh/sshd_config.d/00-linuxonandroid.conf && echo \"KbdInteractiveAuthentication yes\" >> /etc/ssh/sshd_config.d/00-linuxonandroid.conf && echo \"AuthenticationMethods password\" >> /etc/ssh/sshd_config.d/00-linuxonandroid.conf && echo \"StrictModes no\" >> /etc/ssh/sshd_config.d/00-linuxonandroid.conf && echo \"Port 2222\" >> /etc/ssh/sshd_config.d/00-linuxonandroid.conf && echo \"SetEnv PATH=/usr/local/sbin:/usr/local/bin:/usr/sbin:/usr/bin:/sbin:/bin\" >> /etc/ssh/sshd_config.d/00-linuxonandroid.conf && echo \"Subsystem sftp internal-sftp\" >> /etc/ssh/sshd_config.d/00-linuxonandroid.conf && grep -q ^sshd: /etc/passwd || echo \"sshd:x:105:65534:sshd privilege separation:/run/sshd:/usr/sbin/nologin\" >> /etc/passwd && grep -q ^sshd: /etc/shadow || echo \"sshd:*:19700:0:99999:7:::\" >> /etc/shadow && chmod 600 /etc/ssh/ssh_host_*_key 2>/dev/null || true && chmod 755 /etc/ssh /run/sshd /var/run/sshd /var/empty 2>/dev/null || true",
                    launchCommand = "mkdir -p /run/sshd /var/run/sshd /var/empty && sed -i 's/^UsePAM.*/UsePAM no/' /etc/ssh/sshd_config 2>/dev/null || true && sed -i 's/.*pam_systemd.so.*/# &/' /etc/pam.d/sshd /etc/pam.d/common-session 2>/dev/null || true && echo \"UsePAM no\" > /etc/ssh/sshd_config.d/00-linuxonandroid.conf && echo \"PrintMotd no\" >> /etc/ssh/sshd_config.d/00-linuxonandroid.conf && echo \"PrintLastLog no\" >> /etc/ssh/sshd_config.d/00-linuxonandroid.conf && echo \"PermitRootLogin yes\" >> /etc/ssh/sshd_config.d/00-linuxonandroid.conf && echo \"PasswordAuthentication yes\" >> /etc/ssh/sshd_config.d/00-linuxonandroid.conf && echo \"KbdInteractiveAuthentication yes\" >> /etc/ssh/sshd_config.d/00-linuxonandroid.conf && echo \"AuthenticationMethods password\" >> /etc/ssh/sshd_config.d/00-linuxonandroid.conf && echo \"StrictModes no\" >> /etc/ssh/sshd_config.d/00-linuxonandroid.conf && echo \"Port 2222\" >> /etc/ssh/sshd_config.d/00-linuxonandroid.conf && echo \"SetEnv PATH=/usr/local/sbin:/usr/local/bin:/usr/sbin:/usr/bin:/sbin:/bin\" >> /etc/ssh/sshd_config.d/00-linuxonandroid.conf && echo \"Subsystem sftp internal-sftp\" >> /etc/ssh/sshd_config.d/00-linuxonandroid.conf && grep -q ^sshd: /etc/passwd || echo \"sshd:x:105:65534:sshd privilege separation:/run/sshd:/usr/sbin/nologin\" >> /etc/passwd && grep -q ^sshd: /etc/shadow || echo \"sshd:*:19700:0:99999:7:::\" >> /etc/shadow && chmod 600 /etc/ssh/ssh_host_*_key 2>/dev/null || true && chmod 755 /etc/ssh /run/sshd /var/run/sshd /var/empty 2>/dev/null || true && (killall sshd 2>/dev/null || true) && /usr/sbin/sshd -p 2222",
