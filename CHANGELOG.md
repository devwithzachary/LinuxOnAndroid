...
- **Arch Linux ARM Pacman Sandboxing & 1-Click Installation Fix**: Resolved pacman package installation and database synchronization errors on Arch Linux ARM containers (`restricting filesystem access failed because Landlock is not supported by the kernel`, `switching to sandbox user 'alpm' failed`, and `GPGME error: Invalid crypto engine`). Automatically disabled Landlock and sandbox syscalls/users in `/etc/pacman.conf`, bypassed PRoot crypto keyring restrictions (`SigLevel = Never`), force-refreshed rolling release package databases (`pacman -Syy`), replaced fragile multiline heredocs with reliable `printf` generation for `/etc/vnc/xstartup`, and introduced Arch-tailored TigerVNC launch commands with password provisioning and binary verification checklists.
- **Kali Linux DNS Resolver & 1-Click Package Installation Fix**: Resolved package download failures on Kali NetHunter containers (`Temporary failure resolving 'http.kali.org'`, `Unable to locate package xfce4`) caused by unreachable build-server nameserver configurations (`213.186.33.99`) in the upstream Kali archive. Filtered out invalid and internal stub nameservers across `RootfsManager`, passed container-specific target directories during setup and first launch, auto-healed `/etc/resolv.conf` with reliable fallback nameservers (`8.8.8.8`, `1.1.1.1`, `8.8.4.4`) before running package managers, and added root password provisioning to Kali's TigerVNC launcher.
- **Void Linux XBPS Self-Update & 1-Click Package Installation Fix**: Resolved package installation failures on Void Linux containers where the package manager rejected installs with "The 'xbps' package must be updated, please run 'xbps-install -u xbps'", and fixed VNC runtime errors ("vncserver couldn't find 'xinit' on your PATH"). Automatically updated the XBPS package manager before running package installations (both during first launch and 1-click preset execution), included modern `libstdc++` runtime libraries to avoid CXXABI symbol lookup failures, exported standard PATH variables across all shell sessions, deployed a dedicated PRoot-compatible TigerVNC server wrapper eliminating dependencies on Linux virtual terminals (`xinit`), replaced multiline heredocs with reliable `printf` generation for `/etc/vnc/xstartup` and `/usr/bin/bwrap`, ensured correct directory creation for `/root/.vnc` and `/etc/skel/.vnc` prior to password setup, and configured Void-specific TigerVNC launch commands and binary verification checklists.
- **1-Click Software Installation Log Deduplication**: Resolved an issue where completing a 1-click software package installation in the container terminal printed "Installation completed successfully!" twice in the log viewer. Streamlined the step-state flow in `MainViewModel` to deduplicate completion log lines while preserving full terminal log history and real-time status updates.
...
- **Container Deletion Screen & Real-Time Progress Feedback**: Resolved an issue where confirming container deletion instantly returned to the dashboard while gigabytes of rootfs files were still being purged in the background, leaving the deleted container visible on the dashboard for several seconds. Introduced a dedicated full-screen deleting status view with pulsing visual feedback, dynamic step updates ("Closing active terminal sessions", "Purging rootfs files, packages, and storage"), back-gesture protection, and an automatic transition back to the dashboard once deletion and state refresh are 100% complete.
